...
import { Shield, MapPin, Calendar, User, IndianRupee, Cpu, Download, CheckCircle2, Loader2, AlertTriangle } from "lucide-react";
...
const ClaimReportModal = ({ claim, open, onClose }: Props) => {
  const now = new Date();
  const { approveClaim } = useClaims();
  const reportRef = useRef<HTMLDivElement>(null);
  const [downloading, setDownloading] = useState(false);

  const handleDownloadPdf = useCallback(async () => {
    if (!reportRef.current || !claim) return;
    setDownloading(true);
    try {
      await exportElementToPdf(reportRef.current, `PMFBY-Claim-${claim.id}.pdf`);
      toast.success("PDF downloaded", {
        description: `Saved as PMFBY-Claim-${claim.id}.pdf`,
      });
    } catch (e) {
  ...
  const handleApprove = () => {
    ...
    toast.success(`Claim ${claim.id} approved`, {
      description: `Payout of ₹${claim.estCompensation.toLocaleString("en-IN")} authorised.`,
    });
    onClose();
  };
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto p-0">
        ...
        <div ref={reportRef} className="bg-card">
          {/* Tricolor line */}
